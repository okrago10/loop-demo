...
describe("dayPeriodOf", () => {
  it("その時刻を含むローカルの1日を返す", () => {
    const period = dayPeriodOf(local(2026, 8, 13, 14, 30));
  ...
  it("00:00 ちょうどでもその日になる（境界）", () => {
    const period = dayPeriodOf(local(2026, 8, 13, 0, 0));

    expect(period.start).toEqual(local(2026, 8, 13, 0, 0));
  });

  it("23:59 でもその日になる（境界）", () => {
    const period = dayPeriodOf(local(2026, 8, 13, 23, 59));
  ...
  it("月末は翌月1日が終わりになる（境界）", () => {
    const period = dayPeriodOf(local(2026, 8, 31, 12, 0));

    expect(period.end).toEqual(local(2026, 9, 1, 0, 0));
  });

  it("年末は翌年1月1日が終わりになる（境界）", () => {
    const period = dayPeriodOf(local(2026, 12, 31, 12, 0));
...
describe("parseDayPeriod", () => {
  it("YYYY-MM-DD をローカルの1日として解釈する", () => {
    const period = parseDayPeriod("2026-08-13");
  ...
  it("うるう日を受け付ける（境界）", () => {
    expect(parseDayPeriod("2024-02-29").start).toEqual(local(2024, 2, 29, 0, 0));
  });
  ...
  it.each([
    ...
    expect(() => parseDayPeriod(value)).toThrow();
  });
});

describe("formatDay", () => {
  it("ローカルの暦日を YYYY-MM-DD で表す", () => {
    expect(formatDay(local(2026, 8, 13, 14, 30))).toBe("2026-08-13");
  });

  it("1桁の月日は0埋めする", () => {
    expect(formatDay(local(2026, 1, 5))).toBe("2026-01-05");
  });

  it("parseDayPeriod の結果を戻すと元の文字列になる", () => {
    expect(formatDay(parseDayPeriod("2026-03-07").start)).toBe("2026-03-07");
  });
});
