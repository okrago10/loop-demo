...
const WEEK = weekPeriodOf(local(13, 12), { timeZone: RUNTIME_TZ });
...
describe("summarizeWeek の骨格", () => {
  it("7日分の列を返す", () => {
    const summary = summarizeWeek(ENTRIES, WEEK, AFTER_WEEK, RUNTIME_TZ);
  ...
  it("記録が1件も無ければタグの行は空、合計は 0（境界）", () => {
    const summary = summarizeWeek([], WEEK, AFTER_WEEK, RUNTIME_TZ);
...
describe("summarizeWeek のクロス集計", () => {
  ...
  it("タグなしの時間が無ければ undefined（境界）", () => {
    ...
    expect(summarizeWeek(tagged, WEEK, AFTER_WEEK, RUNTIME_TZ).untagged).toBeUndefined();
  });
  ...
  it("週の外の記録は数えない", () => {
    ...
    expect(summarizeWeek(outside, WEEK, AFTER_WEEK, RUNTIME_TZ).totalMs).toBe(4 * HOUR);
  });

  it("日を跨ぐ記録はそれぞれの曜日に分けて数える（境界）", () => {
    // 火 23:00 開始、水 01:00 終了。火に1時間、水に1時間
    const across = [entry("x", local(11, 23), local(12, 1), ["work"])];
    const summary = summarizeWeek(across, WEEK, AFTER_WEEK, RUNTIME_TZ);
    const work = summary.byTag.find((row) => row.tag === "work");
  ...
  it("週の初日の 00:00 に始まる記録を数える（境界）", () => {
    ...
    expect(summarizeWeek(atStart, WEEK, AFTER_WEEK, RUNTIME_TZ).dailyTotalMs[0]).toBe(HOUR);
  });

  it("週の終わりの直前に終わる記録を数える（境界）", () => {
    const atEnd = [entry("x", local(16, 23), local(17, 0), ["work"])];

    expect(summarizeWeek(atEnd, WEEK, AFTER_WEEK, RUNTIME_TZ).dailyTotalMs[6]).toBe(HOUR);
  });

  it("週の終わりちょうどに始まる記録は数えない（境界: 半開区間）", () => {
    const atEnd = [entry("x", local(17, 0), local(17, 1), ["work"])];

    expect(summarizeWeek(atEnd, WEEK, AFTER_WEEK, RUNTIME_TZ).totalMs).toBe(0);
  });
});
...
describe("summarizeWeek と実行中エントリ（終端がない）", () => {
  ...
  it("週より前に始まって、まだ終わっていない記録を数える", () => {
    ...
    const summary = summarizeWeek(running, WEEK, MID_WEEK, RUNTIME_TZ);
    const work = summary.byTag.find((row) => row.tag === "work");
  ...
  it("週より後に始まった実行中の記録は数えない", () => {
    ...
    expect(summarizeWeek(running, WEEK, local(20, 12), RUNTIME_TZ).totalMs).toBe(0);
  });

  it("開始が週の初日とちょうど一致する実行中の記録を数える（境界）", () => {
    const running = [entry("r", local(10, 0), undefined, ["work"])];

    expect(summarizeWeek(running, WEEK, local(10, 1), RUNTIME_TZ).dailyTotalMs[0]).toBe(HOUR);
  });

  it("開始が週の終わりとちょうど一致する実行中の記録は数えない（境界）", () => {
    const running = [entry("r", local(17, 0), undefined, ["work"])];

    expect(summarizeWeek(running, WEEK, local(17, 5), RUNTIME_TZ).totalMs).toBe(0);
  });
  ...
  it("週がまだ始まっていなければすべて 0（境界）", () => {
    ...
    const summary = summarizeWeek(running, WEEK, local(9, 12), RUNTIME_TZ);
