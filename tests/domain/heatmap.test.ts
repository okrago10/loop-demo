...
/** 2026-08-10（月）から始まる週。 */
const WEEK = weekPeriodOf(local(12, 12), { weekStartsOn: 1 });
...
describe("形（必ず7日×24時間）", () => {
  it("記録が無くても7日分の行を返す（境界: 0件）", () => {
    const heatmap = summarizeHeatmap([], WEEK, AFTER);

    expect(heatmap.rows).toHaveLength(7);
  });

  it("各行が 24 件の時間帯を持つ", () => {
    for (const row of summarizeHeatmap([], WEEK, AFTER).rows) {
  ...
  it("開始曜日を変えると行の並びも変わる", () => {
    const sunday = weekPeriodOf(local(12, 12), { weekStartsOn: 0 });

    expect(summarizeHeatmap([], sunday, AFTER).rows[0]?.day.getTime()).toBe(local(9, 0).getTime());
  });
});

describe("時間帯への振り分け", () => {
  it("1時間ちょうどの記録は、その時間帯だけに入る", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 9), local(12, 10))], WEEK, AFTER);
  ...
  it("**時間帯をまたぐ記録は分かれる**", () => {
    ...
    const heatmap = summarizeHeatmap([entry(local(12, 9, 30), local(12, 11))], WEEK, AFTER);
  ...
  it("1時間に満たない記録もその時間帯に入る", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 14, 10), local(12, 14, 25))], WEEK, AFTER);
...
describe("境界", () => {
  it("0時ちょうどに始まる記録は 0 時台に入る（境界: 下限）", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 0), local(12, 1))], WEEK, AFTER);

    expect(cell(heatmap, 2, 0)).toBe(HOUR);
  });

  it("23時台の記録は 23 時に入る（境界: 上限）", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 23), local(13, 0))], WEEK, AFTER);
  ...
  it("**日を跨ぐ記録は両日に分かれる**（境界: 日跨ぎ）", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 23), local(13, 1))], WEEK, AFTER);
  ...
  it("長さ 0 の記録は何も足さない（境界: 同時刻）", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 9), local(12, 9))], WEEK, AFTER);
  ...
  it("週の開始ちょうどに始まる記録が入る（境界: 範囲の下限）", () => {
    const heatmap = summarizeHeatmap([entry(local(10, 0), local(10, 1))], WEEK, AFTER);

    expect(cell(heatmap, 0, 0)).toBe(HOUR);
  });

  it("週の終わりちょうどに終わる記録が入る（境界: 範囲の上限）", () => {
    const heatmap = summarizeHeatmap([entry(local(16, 23), local(17, 0))], WEEK, AFTER);

    expect(cell(heatmap, 6, 23)).toBe(HOUR);
  });

  it("週より前に終わった記録は含まれない（範囲外）", () => {
    const heatmap = summarizeHeatmap([entry(local(9, 9), local(9, 10))], WEEK, AFTER);

    expect(heatmap.totalMs).toBe(0);
  });

  it("週より後に始まった記録は含まれない（範囲外）", () => {
    const heatmap = summarizeHeatmap([entry(local(17, 9), local(17, 10))], WEEK, AFTER);

    expect(heatmap.totalMs).toBe(0);
  });

  it("週をまたいで始まった記録は、週の中の分だけ数える（境界: 範囲の下限を跨ぐ）", () => {
    const heatmap = summarizeHeatmap([entry(local(9, 23), local(10, 1))], WEEK, AFTER);
...
describe("終端のないデータ（実行中エントリ）", () => {
  it("**範囲より前に始まって、まだ終わっていない**", () => {
    const heatmap = summarizeHeatmap([entry(local(9, 23))], WEEK, local(10, 1));

    expect(cell(heatmap, 0, 0)).toBe(HOUR);
  });

  it("範囲の中で始まって、まだ終わっていない", () => {
    const heatmap = summarizeHeatmap([entry(local(12, 9))], WEEK, local(12, 10, 30));
  ...
  it("範囲より後に始まっている（含まれない）", () => {
    const heatmap = summarizeHeatmap([entry(local(17, 9))], WEEK, local(17, 10));

    expect(heatmap.totalMs).toBe(0);
  });

  it("開始時刻が範囲の境界とちょうど一致する", () => {
    const heatmap = summarizeHeatmap([entry(local(10, 0))], WEEK, local(10, 1));
  ...
  it("**まだ経っていない時間は数えない**", () => {
    ...
    const heatmap = summarizeHeatmap([entry(local(12, 9))], WEEK, local(12, 10));
  ...
  it("週がまだ始まっていなければ空（境界）", () => {
    expect(summarizeHeatmap([entry(local(12, 9))], WEEK, local(9, 12)).totalMs).toBe(0);
  });
});
