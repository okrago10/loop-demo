...
describe("決まった形式で返る（DoD）", () => {
  it("同じ日なら時刻だけを返す", () => {
    ...
    expect(formatMoment(local(2026, 8, 12, 9, 30, 45), now)).toBe("09:30:45");
  });

  it("別の日なら日付も付ける", () => {
    const now = local(2026, 8, 12, 18);

    expect(formatMoment(local(2026, 8, 11, 9, 30, 45), now)).toBe("2026-08-11 09:30:45");
  });

  it("桁は常に揃う（1桁の時刻でも 0 で埋める）", () => {
    const now = local(2026, 8, 12, 18);

    expect(formatMoment(local(2026, 8, 12, 1, 2, 3), now)).toBe("01:02:03");
  });

  it("日付の桁も揃う", () => {
    const now = local(2026, 8, 12, 18);

    expect(formatMoment(local(2026, 1, 2, 3, 4, 5), now)).toBe("2026-01-02 03:04:05");
  });
  ...
  it("**秒は落とさない**", () => {
    ...
    expect(formatMoment(local(2026, 8, 12, 9, 30, 7), now)).toBe("09:30:07");
  });

  it("翌年・翌月でも日付が付く", () => {
    const now = local(2026, 8, 12, 18);

    expect(formatMoment(local(2027, 8, 12, 9), now)).toBe("2027-08-12 09:00:00");
    expect(formatMoment(local(2026, 9, 12, 9), now)).toBe("2026-09-12 09:00:00");
  });
});
...
describe("ミリ秒を出さない（DoD）", () => {
  it("ミリ秒を持つ時刻でも、出力に現れない", () => {
    ...
    expect(formatMoment(at, local(2026, 8, 12, 18))).toBe("09:30:45");
  });
  ...
  it("小数点そのものが出ない", () => {
    ...
    expect(formatMoment(at, local(2026, 8, 12, 18))).not.toContain(".");
  });

  it("**保存形式の名残（T / Z）が出ない**", () => {
    const shown = formatMoment(local(2026, 8, 11, 9, 30, 45), local(2026, 8, 12, 18));
...
/**
 * **タイムゾーンを固定して確かめる。** CI は UTC で走るので、ローカルのまま比べると
 * 「UTC のまま出している実装」と区別が付かない。`TZ` は `afterEach` で戻す。
 */
function withZone(zone: string, moment: string, now: string): string {
  process.env["TZ"] = zone;

  return formatMoment(new Date(moment), new Date(now));
}
...
describe("当日と別日の境界（DoD）", () => {
  ...
  it("その日の 00:00:00 は当日（境界: 下限）", () => {
    expect(formatMoment(local(2026, 8, 12, 0, 0, 0), now)).toBe("00:00:00");
  });

  it("その日の 23:59:59 は当日（境界: 上限）", () => {
    expect(formatMoment(local(2026, 8, 12, 23, 59, 59), now)).toBe("23:59:59");
  });

  it("**前日の 23:59:59 は別日**（境界: 1秒違い）", () => {
    expect(formatMoment(local(2026, 8, 11, 23, 59, 59), now)).toBe("2026-08-11 23:59:59");
  });

  it("**翌日の 00:00:00 は別日**（境界: 1秒違い）", () => {
    expect(formatMoment(local(2026, 8, 13, 0, 0, 0), now)).toBe("2026-08-13 00:00:00");
  });
  ...
  it("`now` が日を跨ぐと、同じ時刻の見え方が変わる", () => {
    ...
    expect(formatMoment(at, local(2026, 8, 12, 23, 30))).toBe("23:00:00");
    expect(formatMoment(at, local(2026, 8, 13, 0, 30))).toBe("2026-08-12 23:00:00");
  });

  it("`now` ちょうどでも当日として出る（境界: 同時刻）", () => {
    expect(formatMoment(now, now)).toBe("12:00:00");
  });

  it("月をまたぐ前日も別日として出る（境界: 月末）", () => {
    expect(formatMoment(local(2026, 7, 31, 23), local(2026, 8, 1, 1))).toBe("2026-07-31 23:00:00");
  });
});

describe("`formatClock` は変えていない（log の表示を動かさない）", () => {
  it("`HH:MM` のまま", () => {
    expect(formatClock(local(2026, 8, 12, 9, 30, 45))).toBe("09:30");
  });
});
