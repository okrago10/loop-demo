...
async function startAt9(description = "前の作業 #work"): Promise<void> {
  await createStartCommand(deps(local(9, 0)), testLoadConfig()).run([description], io);
  out = [];
}
...
describe("switch（実行中があるとき）", () => {
  it("前のエントリが確定し、新しいエントリが実行中になる", async () => {
    ...
    await createSwitchCommand(deps(local(10, 30)), testLoadConfig()).run(["次の作業 #会議"], io);
  ...
  it("実行中は新しいエントリ1件だけになる", async () => {
    ...
    await createSwitchCommand(deps(local(10, 30)), testLoadConfig()).run(["次の作業"], io);
  ...
  it("--at で切り替え時刻を指定できる", async () => {
    ...
    await createSwitchCommand(deps(local(12, 0)), testLoadConfig()).run(
      ["次の作業", "--at", "10:30"],
      io,
    );
  ...
  it("引数なしでも切り替えられる（名前なしの新エントリ）", async () => {
    ...
    await createSwitchCommand(deps(local(10, 30)), testLoadConfig()).run([], io);
  ...
  it("前の開始と同時刻に切り替えても失敗しない（長さ0で確定・境界）", async () => {
    ...
    await createSwitchCommand(deps(local(9, 0)), testLoadConfig()).run(["次の作業"], io);
...
describe("switch（実行中がないとき）", () => {
  it("単なる start として動作する", async () => {
    await createSwitchCommand(deps(local(9, 0)), testLoadConfig()).run(["最初の作業 #work"], io);
  ...
  it("エラーにしない（start と同じ扱い）", async () => {
    await expect(
      createSwitchCommand(deps(local(9, 0)), testLoadConfig()).run(["最初の作業"], io),
    ).resolves.toBeUndefined();
  });

  it("記録が1件も無い状態でも落ちない（境界）", async () => {
    await createSwitchCommand(deps(local(9, 0)), testLoadConfig()).run([], io);
...
describe("switch（途中で失敗しても中途半端な状態を残さない）", () => {
  it("新規の追記が失敗したら、前のエントリは実行中のまま戻る", async () => {
    ...
    await expect(
      createSwitchCommand(deps(local(10, 30), storeWithFailingAppend(store)), testLoadConfig()).run(
        ["次の作業"],
        io,
      ),
    ).rejects.toThrow();
  ...
  it("確定の書き込みが失敗しても、実行中が2件になることはない", async () => {
    ...
    await expect(
      createSwitchCommand(deps(local(10, 30), storeWithFailingUpdate(store)), testLoadConfig()).run(
        ["次の作業"],
        io,
      ),
    ).rejects.toThrow();
  ...
  it("--at が前の開始より前なら UserError で、記録は何も変わらない", async () => {
    ...
    await expect(
      createSwitchCommand(deps(local(10, 0)), testLoadConfig()).run(
        ["次の作業", "--at", "08:00"],
        io,
      ),
...
describe("switch の引数", () => {
  it.each([
    ...
    await expect(
      createSwitchCommand(deps(local(10, 0)), testLoadConfig()).run([token], io),
    ).rejects.toThrow(UserError);
  });
  ...
  it("未知のオプションで失敗しても記録は変わらない", async () => {
    ...
    await Promise.resolve(
      createSwitchCommand(deps(local(10, 0)), testLoadConfig()).run(["--help"], io),
    ).catch(() => undefined);
  ...
  it("作業名の中の `/` は弾かない", async () => {
    ...
    await createSwitchCommand(deps(local(10, 0)), testLoadConfig()).run(["a/b の設計"], io);
...
describe("switch を続けて使う", () => {
  it("3回切り替えると3件が隙間なく繋がる", async () => {
    await startAt9();
    await createSwitchCommand(deps(local(10, 0)), testLoadConfig()).run(["2番目"], io);
    await createSwitchCommand(deps(local(11, 0)), testLoadConfig()).run(["3番目"], io);
...
describe("switch は巻き戻しにも失敗したとき、両方の原因を伝える", () => {
  it("追記の失敗と巻き戻しの失敗の両方がメッセージに載る", async () => {
    await createStartCommand(deps(local(9, 0), store), testLoadConfig()).run(["1本目 #work"], io);

    await expect(
      createSwitchCommand(
        deps(local(10, 30), storeWithFailingAppendAndRollback(store)),
        testLoadConfig(),
      ).run(["次の作業"], io),
