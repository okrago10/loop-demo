 ...
 *
 * **受け付ける値の範囲は以前と同じ。** ただし走査が宣言の順から左から右に変わったので、
 * オプションが隣り合って両方とも誤っている場合に、先に報告されるものが変わることがある
 * （`--limit --period 7` は以前「`--limit` には値が必要です」、今は「（`--period` が
 * 続いています）」が付く）。どちらも同じオプションを名指しし、終了コードは 1 のまま。
 *
